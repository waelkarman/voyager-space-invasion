...
public class Boss2Level extends Alien implements Runnable{
    private String imagePath = "./src/main/java/SoEproj/Resource/Boss2Image.gif";
        ...
        while(isVisible()){
            synchronized(aliens){
                if (life < TOT_LIFE * 0.5 && aliens.size() == 1) {  // the aliens must appear only if there aren't other aliens 
                    aliens.add(new MediumAlien(x, y - 30, "BossHelper", goDown));
                    aliens.add(new MediumAlien(x, y, "BossHelper", goDown));
                    aliens.add(new MediumAlien(x, y + 30, "BossHelper", goDown));
                }
            }
